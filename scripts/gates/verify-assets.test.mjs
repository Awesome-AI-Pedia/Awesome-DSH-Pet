...
// 素材全量契约：每个角色必须含全部 15 状态（STATE_NAMES），缺一即拒。
...
test('接受：Codex atlas 行声明 row + rows', () => {
  const good = fullStates({ idle: { sheet: 'idle.png', row: 0, rows: 9, frames: 2, fps: 4, playback: 'loop' } })
...
test('拒绝：Codex atlas row 缺 rows 或越界', () => {
  const bad = fullStates({
    idle: { sheet: 'idle.png', row: 0, frames: 2, fps: 4, playback: 'loop' },
    walk: { sheet: 'walk.png', row: 9, rows: 9, frames: 2, fps: 4, playback: 'loop' },
  })
  const root = makeTree({ 'lib/assets/manifest.json': bad })
  writeSheets(root, 'lib/assets')
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /声明 row 时 rows 必须是正整数/)
  assert.match(errors.join('\n'), /row 9 必须小于 rows 9/)
})
...
test('接受：角色索引（characters + default）且 sheet 在角色目录', () => {
  const good = {
    characters: {
      'awesome-dsh-pet': { meta: { stageSize: 110 }, states: fullStates().states },
      cat: { states: fullStates().states },
    },
    default: 'awesome-dsh-pet',
  }
  const root = makeTree({ 'lib/assets/manifest.json': good })
  writeSheets(root, 'lib/assets/characters/awesome-dsh-pet')
...
test('拒绝：角色 sheet 文件缺失（期望在 characters/<id>/ 下）', () => {
  const bad = {
    characters: { 'awesome-dsh-pet': { states: fullStates().states } },
    default: 'awesome-dsh-pet',
  }
  const root = makeTree({ 'lib/assets/manifest.json': bad, 'assets/idle.png': 'x' }) // 平铺不算数
  const { ok, errors } = check(root)
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /文件不存在.*characters\/awesome-dsh-pet\/idle\.png/)
})
...
test('接受：顶层 states 简写（sheet 平铺 assets/）兼容', () => {
  const root = makeTree({ 'lib/assets/manifest.json': fullStates() })
  writeSheets(root, 'lib/assets')
  const { ok, errors } = check(root)
  assert.equal(ok, true, errors.join('\n'))
