...
function checkStates(states, roleId, errors, root) {
  ...
  for (const name of STATE_NAMES) {
    if (!declared.has(name)) {
      errors.push(`${label}: 缺必备状态 ${name}（素材必须全量提供 15 状态）`)
    }
  }
  for (const [name, cfg] of Object.entries(states)) {
    ...
    if (cfg.row !== undefined) {
      if (!Number.isInteger(cfg.row) || cfg.row < 0) {
        errors.push(`${label}.${name}: row 必须是非负整数`)
      }
      if (!Number.isInteger(cfg.rows) || cfg.rows < 1) {
        errors.push(`${label}.${name}: 声明 row 时 rows 必须是正整数`)
      } else if (Number.isInteger(cfg.row) && cfg.row >= cfg.rows) {
        errors.push(`${label}.${name}: row ${cfg.row} 必须小于 rows ${cfg.rows}`)
      }
    }
    // 播放模式：playback 枚举 + 帧数交叉校验。
...
export function check(root = ROOT) {
  ...
  // 角色索引（characters）优先；顶层 states 若并存也校验，
