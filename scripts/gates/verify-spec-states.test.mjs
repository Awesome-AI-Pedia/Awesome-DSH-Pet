...
// 权威集合使用 STATE_NAMES（素材全量契约），校验 STATE_TABLE 行漂移，
...
test('接受：spec 播放行为列与 manifest playback 一致', () => {
  const manifest = { characters: { 'awesome-dsh-pet': { states: Object.fromEntries(names.map((s) => [s, { sheet: `${s}.png`, frames: 2, fps: 4, playback: PLAYBACK[s] }])) } }, default: 'awesome-dsh-pet' }
...
test('拒绝：spec 播放行为列 ≠ manifest playback（语义级配错：idle 配 loop）', () => {
  ...
  assert.match(errors.join('\n'), /characters\.awesome-dsh-pet\.states\.idle: manifest playback "blink" ≠ spec 状态总表播放行为 "loop"/)
})
