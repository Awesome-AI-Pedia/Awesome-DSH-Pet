...
const ROUTES_SRC = `export const ROUTE_PREFIX = '/awesome-dsh-pet'
...
test('拒绝：client 手写路由字面量（单一来源被破坏）', () => {
  const { ok, errors } = check(makeTree({ client: "const STATE_PATH = '/awesome-dsh-pet/state'" }))
  assert.equal(ok, false)
  assert.match(errors.join('\n'), /client\/index\.mjs 手写路由前缀字面量/)
})

test('拒绝：index.mjs 未从 routes.mjs import（端点散落）', () => {
  const { ok, errors } = check(makeTree({ node: "export const STATE_PATH = '/awesome-dsh-pet/state'" }))
  assert.equal(ok, false)
