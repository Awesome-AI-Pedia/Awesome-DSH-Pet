...
const SNAP = `(() => {
  const sprite = document.querySelector('[data-awesome-dsh-pet] .pet-sprite')
  const stage = document.querySelector('[data-awesome-dsh-pet] .pet-stage')
...
// 走真实时间（sleepAfterMs=60000 + 3s 轮询）；断言序列是交互醒觉契约：
...
async function sleepDragWake({ call, log }) {
  ...
  const r0 = await call('Runtime.evaluate', { expression: `(() => {
    const hit = document.querySelector('[data-awesome-dsh-pet] .pet-hitarea')
