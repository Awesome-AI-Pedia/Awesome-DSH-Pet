...
// 断言：无 "Failed to load plugins"（client apply 未抛错）；[data-awesome-dsh-pet] 存在；
...
function analyze(html) {
  ...
  // 元素 vs CSS：host 元素是 <div data-awesome-dsh-pet=""（属性带 =），CSS 是 [data-awesome-dsh-pet]。
  if (!/<div data-awesome-dsh-pet[=> ]/.test(html)) errors.push('未找到 [data-awesome-dsh-pet] 元素（client half 未 apply）')
  ...
  // transform 合法性：sprite 的 transform 必须是合法 scale 数值。
...
// dump 改 CDP 真实时间模式：`--virtual-time-budget` 与 SSE 长连接（/awesome-dsh-pet/events
