...
// 零负反馈：无 hunger/mood 属性状态；情绪只由事件瞬发 + 互动喜悦。
...
test('零负反馈：pet 数值字段不驱动状态（无 hunger/mood 分支）', () => {
...
test('工作陪伴：workingActive 插曲 > think 常态，不依赖 activity', () => {
...
test('回合完成庆祝：celebrateUntil 窗口内播 celebrate，且不抢互动/等待', () => {
...
test('STATE_TABLE 全序由 verify-spec-states 门禁守护（state-machine.md 优先级列表 ↔ 行序）', () => {
  // 行序一致性由门禁机械校验；这里不复制完整 order 数组，避免出现第三份状态顺序。
...
test('交互醒觉集成：sleep → drag → 放下（sleeping 归零）→ 缓冲过期后不回 sleep', () => {
  ...
  // 对照：若 sleeping 未归零 → 缓冲过期立即回 sleep
...
test('交互醒觉：feed/play 结束后同样不回 sleep（sleeping 已归零）', () => {
  ...
  // 对照：若 sleeping 未归零 → joy 结束后立即回 sleep
