...
// stateOf 缺状态返回 undefined（调用方占位），isKnownState 基于 STATE_NAMES。
...
const COMPACT_MANIFEST = {
  states: { idle: { sheet: 'idle.png', frames: 3, fps: 2, loop: true } },
}
const NEW_MANIFEST = {
  characters: {
    'awesome-dsh-pet': { meta: { stageSize: 110 }, states: { idle: { sheet: 'idle.png', frames: 3, fps: 2, loop: true } } },
    cat: { meta: { stageSize: 96 }, states: { walk: { sheet: 'walk.png', frames: 3, fps: 6, loop: true } } },
  },
  default: 'cat',
}

test('顶层 states 简写 → awesome-dsh-pet 单角色（兼容读）', () => {
  const { characters, defaultId } = parseCharacters(COMPACT_MANIFEST)
...
test('新格式 characters + default', () => {
  ...
  assert.deepEqual(Object.keys(characters), ['awesome-dsh-pet', 'cat'])
...
test('listCharacters / defaultCharacter / getCharacter', () => {
  assert.deepEqual(listCharacters(NEW_MANIFEST), ['awesome-dsh-pet', 'cat'])
...
test('stateOf：缺状态返回 undefined（调用方显示占位）', () => {
...
test('ROLE_ID_RE：合法 id 通过，非法拒绝（URL 注入面）', () => {
  assert.ok(ROLE_ID_RE.test('awesome-dsh-pet'))
