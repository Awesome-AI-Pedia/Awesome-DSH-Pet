...
export const NAMESPACE = 'awesome-dsh-pet'
...
export const DEFAULTS = Object.freeze({
  ...
  pollMs: 15000,          // /state 轮询间隔（SSE 即时下发事件，轮询只兜底 SSE 断线）
  ...
  imageApi: {             // AI 生成宠物动作变体（方案 B）；用户自填 apiKey 才启用
    enabled: false,       // 总开关（false → client 上传时不问 AI 变体）
    provider: 'dashscope',// 图像生成商 id（当前支持：dashscope；抽象层预留 openai/gemini 扩展点）
    apiKey: '',           // 用户自填的 API key（存 settings.yaml；空字符串视为未配置）
    model: '',            // 模型名（空字符串则由 provider 默认——dashscope=wanx2.1-imageedit）
  },
})
...
export function buildSchema() {
  return z.object({
    ...
    size: z.number().min(64).max(160).default(DEFAULTS.size),
      ...
      imageApi: z.object({
        enabled: z.boolean().default(DEFAULTS.imageApi.enabled),
        provider: z.string().default(DEFAULTS.imageApi.provider),
        apiKey: z.string().default(DEFAULTS.imageApi.apiKey),
        model: z.string().default(DEFAULTS.imageApi.model),
      }),
    })
}
