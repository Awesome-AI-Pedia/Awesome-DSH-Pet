...
var BLINK_MIN_INTERVAL_MS = 8e3;
var BLINK_MAX_INTERVAL_MS = 18e3;
...
var DEFAULT_ROLE_ID = "awesome-dsh-pet";
...
function createPanel({ anchor = "below", variant = "plain", offsetY = 12, zIndex = "3", display = "block", doc = globalThis.document } = {}) {
  const el = doc.createElement("div");
  const pos = anchor === "above" ? `top: -${offsetY}px; transform: translate(-50%, -100%);` : `top: calc(100% + ${offsetY}px); transform: translateX(-50%);`;
...
// lib/src/routes.mjs
var ROUTE_PREFIX = "/awesome-dsh-pet";
...
var UPLOAD_PATH = `${ROUTE_PREFIX}/upload`;
var CHARACTERS_PATH = `${ROUTE_PREFIX}/characters`;
var GENERATE_PATH = `${ROUTE_PREFIX}/generate`;
...
function apply(ctx = {}) {
  ...
  const readCtx = (name2) => typeof ctx.get === "function" ? ctx.get(name2) : ctx[name2];
  const rawResolve = readCtx("resolveAssetUrl");
  const resolveAssetUrl = typeof rawResolve === "function" ? rawResolve : (url) => url;
  const rawStartDrag = readCtx("startWindowDrag");
  const startWindowDrag = typeof rawStartDrag === "function" ? rawStartDrag : null;
  ...
  uploadBtn.textContent = "\u{1F4F8} \u4E0A\u4F20\u7167\u7247";
  ...
  let pressed = false;
  let moved = false;
  let nativeWindowDragActive = false;
  ...
  const showPlaceholder = (name2) => {
    ...
    console.warn(`[awesome-dsh-pet] \u72B6\u6001 ${name2} \u7F3A\u5C11\u53EF\u7528 sheet\uFF08manifest \u5E94\u542B\u5168\u90E8 15 \u72B6\u6001\uFF1B\u82E5\u5DF2\u58F0\u660E\u5219\u7D20\u6750\u52A0\u8F7D\u5931\u8D25\uFF09`);
  };
  const sheetKey = (sheet) => `${characterId}:${sheet}`;
  const characterSheetUrl = (id, sheet) => resolveAssetUrl(`${ASSETS_URL}/characters/${id}/${sheet}`);
  const sheetUrl = (sheet) => characterSheetUrl(characterId, sheet);
  const frameMetrics = (anim, size) => {
    const frames = Number.isInteger(anim.frames) && anim.frames > 0 ? anim.frames : 1;
    const atlasRows = Number.isInteger(anim.rows) && anim.rows > 0 ? anim.rows : Number.isInteger(anim.row) ? 9 : 1;
  ...
  const applyFacing = () => {
    ...
    const { frameW, frameH } = frameMetrics(cfg2, size);
    const target = host.offsetWidth || 110;
    const scale = Math.min(target / frameW, target / frameH, 1);
  ...
  const analyzeSheet = (img, anim) => {
    ...
    ctx2.drawImage(img, 0, row * fh, fw, fh, 0, 0, fw, fh);
  ...
  const preload = (name2, cfg2) => loadImageWithRetry(sheetUrl(cfg2.sheet)).then((img) => {
    ...
    const box = analyzeSheet(img, cfg2);
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      ...
      const pref = (() => {
        try {
          return localStorage.getItem("awesome-dsh-pet:character") ?? null;
        } catch {
          return null;
        }
      })();
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      await Promise.all(Object.entries(target.states).map(([n, cfg2]) => loadImageWithRetry(characterSheetUrl(id, cfg2.sheet)).then((img) => {
        if (img === null) return;
      ...
      try {
        localStorage.setItem("awesome-dsh-pet:character", id);
      } catch {
      }
      transient = null;
      transientUntil = 0;
  ...
  const tick = () => {
    ...
    if (cfg2 && loaded.has(sheetKey(cfg2.sheet))) {
      if (cfg2.playback !== void 0 && !["loop", "pingpong", "once", "blink"].includes(cfg2.playback)) {
        console.warn(`[awesome-dsh-pet] \u72B6\u6001 ${animState} playback "${cfg2.playback}" \u975E\u6CD5\uFF0C\u6309 loop \u64AD\u653E`);
      }
      const size = sheetSize.get(sheetKey(cfg2.sheet));
      const { frameW, frameH, row } = frameMetrics(cfg2, size);
      if (!showingSprite) {
  ...
  hitarea.addEventListener("pointermove", (e) => {
    ...
    if (Math.abs(e.clientX - startX) + Math.abs(e.clientY - startY) > 6) {
      const startingDrag = !moved;
      if (startingDrag && startWindowDrag === null) hitarea.setPointerCapture(e.pointerId);
      moved = true;
      dragging = true;
      transient = null;
      transientUntil = 0;
      joyUntil = 0;
      layoutStatus();
      const nextFlip = e.screenX < lastPointerScreenX ? 1 : -1;
      if (nextFlip !== flip) {
        flip = nextFlip;
      ...
      if (startingDrag && startWindowDrag !== null) {
        nativeWindowDragActive = true;
        dragClickSuppressUntil = Date.now() + 1e3;
        try {
          Promise.resolve(startWindowDrag()).catch((err) => console.warn("[awesome-dsh-pet] \u539F\u751F\u7A97\u53E3\u62D6\u52A8\u5931\u8D25", err));
        } catch (err) {
          console.warn("[awesome-dsh-pet] \u539F\u751F\u7A97\u53E3\u62D6\u52A8\u5931\u8D25", err);
        }
      }
    }
    lastPointerScreenX = e.screenX;
  ...
  hitarea.addEventListener("pointerup", (e) => {
    pressed = false;
    dragging = false;
    const wasMoved = moved || nativeWindowDragActive || Date.now() < dragClickSuppressUntil;
    nativeWindowDragActive = false;
  ...
  const onDragAbort = () => {
    pressed = false;
    if (nativeWindowDragActive) return;
    dragging = false;
    if (moved) {
      dragReleaseUntil = Date.now() + DRAG_RELEASE_MS;
      releaseInteraction();
  ...
  const doUpload = async (file) => {
    ...
    const name2 = promptForName(`\u5BA0\u7269 ${(/* @__PURE__ */ new Date()).toLocaleTimeString().slice(0, 5)}`);
    try {
      const res = await fetch(UPLOAD_PATH, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: name2, mime: "image/png", pngBase64: base64 })
      });
...
\u8981\u7528 AI \u751F\u6210 9 \u4E2A\u52A8\u4F5C\u53D8\u4F53\u5417\uFF1F\uFF08\u7EA6 60 \u79D2\uFF1B\u751F\u6210\u4E2D\u5BA0\u7269\u4F1A\u5B9E\u65F6\u66F4\u65B0\u59FF\u52BF\uFF09`);
  ...
  const triggerGenerate = async (characterId2) => {
    try {
      const res = await fetch(GENERATE_PATH, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ characterId: characterId2 })
      });
      if (!res.ok) {
  ...
  const refreshCharacterAssets = async (id) => {
    try {
      ...
      loaded.clear();
      sheetSize.clear();
      for (const [n, cfg2] of Object.entries(target.states)) {
        const img = await loadImageWithRetry(characterSheetUrl(id, `${cfg2.sheet}?t=${Date.now()}`));
        if (img === null) continue;
        sheetSize.set(`${id}:${cfg2.sheet}`, { w: img.naturalWidth, h: img.naturalHeight });
        loaded.add(`${id}:${cfg2.sheet}`);
        const box = analyzeSheet(img, cfg2);
        if (box !== null) stateBoxes.set(n, box);
      }
      const cur = animState;
