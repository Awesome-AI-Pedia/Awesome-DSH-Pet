...
export function apply(ctx = {}) {
  ...
  // 两种消费形态兼容：
  // - Web（cordis Context）：直接属性访问 `ctx.foo` 触发 inject 守卫（未在 inject
  //   声明就抛 "cannot get property … without inject"），但 inject 列表里的项都是
  //   *必需* 服务——web 组合缺 startWindowDrag（仅 Tauri 桌面提供）就会卡
  //   "waiting for service" 不 activate。绕道 `ctx.get(name)`（cordis 提供的
  //   逃生口）不走守卫、缺席返回 undefined。
  // - 桌面（desktop/src/main.mjs 传裸对象 {resolveAssetUrl, startWindowDrag}）：
  //   裸对象没 `.get`，直接读属性即可。
  const readCtx = (name) => (typeof ctx.get === 'function' ? ctx.get(name) : ctx[name])
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      await Promise.all(Object.entries(target.states).map(([n, cfg]) => loadImageWithRetry(characterSheetUrl(id, cfg.sheet)).then((img) => {
        if (img === null) return // 重试耗尽：该状态保持占位（showPlaceholder 提示）
  ...
  hitarea.addEventListener('pointermove', (e) => {
    ...
    if (Math.abs(e.clientX - startX) + Math.abs(e.clientY - startY) > 6) {
      ...
      if (startingDrag && startWindowDrag !== null) {
        ...
        dragClickSuppressUntil = Date.now() + 1000
        try {
  ...
  const onDragAbort = () => {
    pressed = false
    // Tauri 接管窗口拖动时 WebKit 会发 pointercancel；保留移动状态，等待真实 pointerup 收尾。
  ...
  const refreshCharacterAssets = async (id) => {
    try {
      ...
      for (const [n, cfg] of Object.entries(target.states)) {
        const img = await loadImageWithRetry(characterSheetUrl(id, `${cfg.sheet}?t=${Date.now()}`))
...
// Client module 入口：exports {name, apply, inject} 经 __ModuleLoader__.load 注册。
// ctx 仅消费 sessions（会话感知）；缺席时宠物照常跑，无思考陪伴。
// inject：宿主上下文属性访问守卫要求声明——resolveAssetUrl（assets URL 前缀重写，
// 用于 dev/prod 不同挂载路径）与 startWindowDrag（桌面 Tauri 组合下的窗口拖拽）。
// 均为可选（apply 内已用 typeof 判断兜底），声明后守卫允许属性读取。
// Client module 入口：exports {name, apply} 经 __ModuleLoader__.load 注册。
// ctx 消费 resolveAssetUrl（web 提供）/ startWindowDrag（Tauri 桌面提供），
// 均经 ctx.get(name) 读取以绕开 inject 守卫（inject 声明会让「缺席即视为必需」，
// web 组合缺 startWindowDrag 会卡 "waiting for service" 不 activate）。
