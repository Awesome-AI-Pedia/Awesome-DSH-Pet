...
//   pet 不参与状态选择，动画只由事件、会话和本地交互驱动。
// 状态优先级由 STATE_TABLE 声明（顺序即优先级）。
// Node half 输出事实窗口，client 负责本地交互选择。
// drag/burst/transient 都是临时覆盖；窗口结束后重新计算底层派生状态。
// transient 由宿主计时（TRANSIENT_MS 超时兜底），本模块只做选择；burst 由 activity.until 窗口决定。
// 会话感知：sessionThink/sessionWait 由 host sessions 服务聚合，
// 是持续陪伴底座——任一会话活跃时宠物保持清醒陪伴（覆盖 sleep/walk），低于用户互动与事件反馈。
// 行为节奏：working 不是任务状态指示灯，而是 client 节奏器随机插入的工作插曲
...
export const BLINK_MIN_INTERVAL_MS = 8000  // 眨眼最小间隔
export const BLINK_MAX_INTERVAL_MS = 18000 // 眨眼最大间隔
...
/**
 * 用户交互醒觉决策：拖拽/喂食/玩耍/开菜单都是用户在场信号。
 * 空闲计时从交互时刻重新起算，交互瞬间若正睡着则附加 wake 过渡。
